...
class MLPHierarchicalModel():
    ...
    def __init__(self, input_dim, num_neuron, num_block, num_layer_pb, lamda, use_linear, decay, verbose, random_seed = 1):
        """
        Args:
    ...
    def __build_neural_net(self):
        ...
        if self.use_linear:
            linear_input = self.X
            linear_output = tf.layers.dense(linear_input, 1, kernel_regularizer=tf.contrib.layers.l2_regularizer(float(self.lamda)))
        ...
        for i in range(self.num_layer_pb):
            if i == 0:
                layer = tf.layers.dense(layer, self.num_neuron, tf.nn.relu,
                                        ...
                                        kernel_regularizer=tf.contrib.layers.l1_regularizer(float(self.lamda)))
            else:
                layer = tf.layers.dense(layer, self.num_neuron, tf.nn.relu,
                                        kernel_initializer=tf.contrib.layers.xavier_initializer(seed=1))
